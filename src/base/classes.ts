export default function classes() {
  ...
  dog.move(10)
  dog.bark()
  // 这个例子展示了最基本的继承：类从基类中继承了属性和方法。 这里， Dog是一个 派生类，它派生自 Animal 基类，通过 extends关键字。
  ...
  // 这个例子演示了如何在子类里可以重写父类的方法。 Snake类和 Horse类都创建了 move方法，它们重写了从 Animal继承来的 move方法，
  // 使得 move方法根据不同的类而具有不同的功能。 注意，即使 tom被声明为 Animal类型，但因为它的值是 Horse，调用 tom.move(34)时，它会调用 Horse里重写的方法：
  // Slithering...
  ...
  //   公共，私有与受保护的修饰符
  // 默认为 public
  // 在上面的例子里，我们可以自由的访问程序里定义的成员。 如果你对其它语言中的类比较了解，就会注意到我们在之前的代码里并没有使用 public来做修饰；
  ...
  let a4 = new Animal4('Cat')
  log(a4)
  // a4.name; // 错误: 'name' 是私有的.
  ...
  //  我们创建了几个这些类的实例，并相互赋值来看看会发生什么。 因为 Animal和 Rhino共享了来自 Animal里的私有成员定义 private name: string，因此它们是兼容的。
  ...
  class Octopus2 {
    readonly numberOfLegs: number = 8
    // 参数属性：在构造函数中既是属性定义，也是构造函数的参数，一举两得，但传统OO语言使用者理解上没那么直接
    constructor(readonly name: string) {}
  }
  let o2 = new Octopus2('aaa')
  log(o2, o2.name)
  ...
  let o3 = new Octopus3('bbb')
  log(o3) //o3.name非法，因为name是private
  ...
  // 我们也可以创建类的静态成员，这些属性存在于类本身上面而不是类的实例上。 在这个例子里，我们使用 static定义 origin，因为它是所有网格都会用到的属性。
  ...
  // 上面的代码里， let Greeter将被赋值为构造函数。 当我们调用 new并执行了这个函数后，便会得到一个类的实例。
  // 这个构造函数也包含了类的所有静态属性。 换个角度说，我们可以认为类具有 实例部分与 静态部分这两个部分。

  // 此处与java和其他真正面向对象编程语言不同之处。
  ...
  let greeter31: Greeter3
  greeter31 = new Greeter3()
  console.log(greeter31.greet())
  let greeterMaker: typeof Greeter3 = Greeter3
  greeterMaker.standardGreeting = 'Hey there!'
  let greeter32: Greeter3 = new greeterMaker()
  console.log(greeter32.greet())
  // 这个例子里， greeter31与之前看到的一样。 我们实例化 Greeter类，并使用这个对象。 与我们之前看到的一样。
  ...
  // 不管是接口可以继承类，还是类当成接口使用，都不是真正的OO所倡议的，故应该避免这么使用。不知道语言设计者的初衷是为了平衡什么。
  //   把类当做接口使用
