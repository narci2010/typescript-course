...
// 使用时也要注意保存的状态过大时产生的开销，保存在硬盘上的还好，如果是运行时保存在内存上的，比如一些复杂对象的undo/redo操作，
// 保存每一个状态都是很大的内存开销，这时就需要做些限制，比方设置一个历史记录栈的最大值来限定内存的使用。
...
namespace Memento {
  ...
  class History implements Memento {
    ...
    static defaultMemoItem: MemoItem = {
      command: undefined,
      param: { channelNum: 0 }
    } // undo到第一步时前面没有command了，返回一个默认command

    currPos: number = 0 // 当前undo/redo到了哪一个

    get currIndex(): number {
      // currPos是从后往前的顺序， currIndex是正向顺序
      return this.memoList.length - this.currPos - 1
    }

    set(item: MemoItem) {
      if (this.currPos != 0) {
        // 不是0的话表示已经undo过，往上叠加push前先删除后面的
    ...
    findLastWithSameType(memoItem: MemoItem): MemoItem {
      // 找出上个同类型的command，得到参数，以这个参数来做undo操作，回到之前的状态
  ...
  class UndoRedoManager {
    ...
    push(command: Command, param: any) {
      // command执行时应该push进来
      this.history.set({ command, param })
    }
    redo() {
      if (this.history.currPos == 0) {
        // 表示没undo过，当然redo也没必要了
        return
      }
      let memoItem = this.history.getNext() // 取出上次undo过的下一个command并执行
      this.history.currPos--
      if (memoItem && memoItem.command) memoItem.command!.redo(memoItem.param)
    }
  ...
  abstract class Command implements Executable, UndoRedoable {
    ...
    execute(param: any) {
      UndoRedoManager.instance.push(this, param)
    }
  ...
  class SwitchCommand extends Command {
    execute(param: any) {
      super.execute(param)
      this.controller.switchTo(param.channelNum)
    }
    undo(currParam: any, lastParam: any) {
      this.controller.switchTo(lastParam.channelNum)
    }
  }
  ...
  class Controller {
    ...
    constructor(
      ...
      private closeCmd: Command,
      private switchCmd: Command
    ) {}
