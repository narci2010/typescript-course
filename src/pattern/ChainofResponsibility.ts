...
// 注意：链的实现。
// WebApi的handler可能大家有用过，对发出去的请求和请求回来的数据都可以用自定义handler在发出前或最终回来前进行处理，非常方便，
// 下面用TypeScript来简单实现一个HttpHandler:
// 先建立一个抽象Handler类，包含一个发送请求的sendReqeust以及用来链式处理的innerHandler：
abstract class HttpHandler {
  constructor(protected innerHandler: HttpHandler | undefined) {}
