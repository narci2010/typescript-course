...
// 这种解释器的优点在于各种表达式的解析相对独立，要加入新的规则也不会影响现有的解析。缺点也很明显，一个表达式一个类，复杂语法或复合语法的话表达式数量就非常多，
// 并且表达式之间也很难真正独立。
...
class BeginExpression implements Expression {
  interpret(context: Context) {
    context.isMatch =
      ...
      context.currentPatternExpression === '^' &&
      context.currentText === context.pattern[context.currentPatternIndex + 1]
    context.currentPatternIndex = context.currentPatternIndex + 2
...
class BracketExpression implements Expression {
  interpret(context: Context) {
    ...
    // 取出中括号中的字符串
    let expression = context.pattern.substr(
      prevIndex + 1,
      context.currentPatternIndex - prevIndex - 1
    )
...
class BraceExpression implements Expression {
  interpret(context: Context) {
    ...
    while (context.pattern[++endIndex] !== '}') {
      if (endIndex + 1 === context.pattern.length) {
        throw new Error(`miss symbol }`)
      }
    }
    // 取出大括号中的内容
    let expression = context.pattern.substr(
      context.currentPatternIndex + 1,
      endIndex - context.currentPatternIndex - 1
    )
    //     console.log('expression:' + expression)
    ...
    context.pattern =
      context.pattern.substr(0, context.currentPatternIndex) +
      newExpression +
      context.pattern.substr(endIndex + 1)
    //     console.log('context.pattern :' + context.pattern)
  }
}
...
class StringExpression implements Expression {
  interpret(context: Context) {
    ...
    console.log('StringExpression:' + context.isMatch)
    context.currentPatternIndex++
    context.currentTextIndex++
  }
}
...
class Regex {
  ...
  IsMatch(text: string): boolean {
    ...
    for (
      context.currentPatternIndex = 0;
      context.currentPatternIndex < context.pattern.length;

    ) {
      //  console.log('context.currentPatternIndex:' + context.currentPatternIndex)
      //  console.log('context.currentText:' + context.currentTextIndex)
